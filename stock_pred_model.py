...
def video_title_model_plot(df):
    ...
    mean_scores = title_score_df.groupby(by=title_score_df['searchedDate'].dt.date).mean()
    index_mean_scores = mean_scores.reset_index()
    
    # Get the data for the stock Apple by specifying the stock ticker, start date, and end date 
    ticker_df = yf.download("NVDA", period = "ytd", start=index_mean_scores["searchedDate"].min(), end=index_mean_scores["searchedDate"].max())
    ...
    ax2.plot(full_joined_df.searchedDate, full_joined_df.compound, color="blue",marker="o")
